...
const InputDurationField = observer(({ style, onCommit = () => {} }) => {
    ...
    return (
        <Pressable className={style} onPress={() => setIsPickerVisible(!isPickerVisible)}>
            <DateTimePicker isVisible={isPickerVisible} mode='time' onConfirm={handleDurationPicker} is24Hour={true} locale='es-ES' display='spinner' date={date} onCancel={() => {setIsPickerVisible(false)}}/>
